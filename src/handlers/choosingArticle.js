...
export default async function choosingArticle(params) {
  ...
  if (doesNotContainMyArticle && isNonsenseText(data.searchedText)) {
    ...
    const summary =
      `${count.RUMOR || 0} person(s) consider this to be a rumor ❌\n` +
      `${count.NOT_RUMOR || 0} person(s) think this can be a truth ⭕\n` +
      `${count.OPINIONATED ||
    ...
    if (articleReplies.length !== 0) {
          ...
          elements: articleReplies
            ...
            .map(
              (
                ...
                buttons: [createPostbackAction('Read this reply', idx + 1)],
              })
            ),
        },
      };
      ...
      if (articleReplies.length > 10) {
        replies.push({
          type: 'text',
          content: {
            text: `Please refer to ${getArticleURL(
              selectedArticleId
            )} for more replies.`,
          },
        });
